...
\section{Receiver-Driven Protocol}
     ...
     To avoid the receiver to request a non-exist vertex split (trying to split
     a leaf vertex in the vertex hierarchy),
     an extra bit is attached to every vertex to indicate whether it is a leaf or not.
    ...
    In other words, we use the neighbor faces to estimate the projected area of the 
    bounding sphere on the screen.
    ...
    screen-space area. The effectiveness of this method is evaluated in Section \ref{s:dstream:evaluation},
    compared with other two methods. One is based on the level of a vertex in the 
    vertex hierarchy and one is randomly pick a visible vertex to split.
    ...
    in our experiments, can be decided by the receiver based on mesh size and network bandwidth.
    
    With the screen-area based method, only the visual importance of currently existed
    ...
    be higher than some current existed vertices. Second, if only the vertex splits of 
    current existed vertices are requested, the requests may not fill the pipeline when RTT
    ...
    detect the silhouette. Since we assign a unique color to each faces, we know
    which color does not belong to the mesh. Then, whenever we find a pixel is a 
    neighbor of an ``alien'' color, we know that the corresponding 
    face of this pixel is in silhouette.
    We can then split the neighbors of this pixel disregard of the visibility.
    ...
    without compression. It is already larger than the vertex split corresponding to the ID,
    ...
    %among the packets.
    First, we filter out all the children vertices in the requests.
    ...
    For each non-children vertices, we indicate how many descendants are also requested.
    Only a number to indicate how many levels to go down is enough for this purpose.
