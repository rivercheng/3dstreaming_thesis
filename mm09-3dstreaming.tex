...
move the mesh $x$ units right, $y$ units up, and $z$ units closer (i.e. out of the screen). 
...
\begin{figure}
    \centering
    \epsfig{file=transition.eps, width=0.8\textwidth}
...
\begin{figure}[htdp!]
    \centering
    \epsfig{file=figs/traceHistogram0/UnActionEventFrequency.eps, width=0.5\textwidth}
...
\begin{figure}[htdp!]
    \centering
    \epsfig{file=cont_prob_9.eps, width=0.5\textwidth}
...
\begin{figure}
    \centering
    \epsfig{file=thai_2.eps, width=0.7\textwidth}
...
\begin{figure}
    \centering
    \epsfig{file=sample_size_1.eps, width=0.5\textwidth}
...
\begin{figure}
    \centering
    \epsfig{file=sample_size_2.eps, width=0.5\textwidth}
...
The state is the Markov chain can be the viewpoint, represented as a 6-tuple: {x, y, z, ax, ay, az}
...
We can reduce the second order Markov chain to be first order by defining the current state as
...
As a result, we can assume the transition probability only depends on the current state.
...
It is based on a simplified model, which does not directly derive the transition matrix and follow it.
...
To simplify the model, we need more understanding of user behaviors.
According to our observation, continuing the previous action has the highest probability in most cases. 
We think it is mainly because users tend to press the same key multiple times. If the user has decided to 
change to a new action, then the previous action may have no effect on the next action.

Therefore, we categorized the action to two groups: \textit{continue}, and \textit{change}. 
We determine the next action in two steps:
\begin{enumerate}
    \item We determine the probability of ``continue'' and ``change'',
i.e. $Pr(continue|_{current state})$ and $Pr(change|_{current state})$ respectively. Apparently, 
we have
\[
Pr(change|_{current state}) = 1 - Pr(continue|_{current_state}).
\]
    \item We decide the distribution of $A_n$ if ``change'' is decided, i.e. $Pr(A_n |_{current state, A_n != A_p})$
for each possible $A_n$. 
\end{enumerate}
We assume that only step 1 depends on the previous action, and step 2
only depends on ($x,y,z,ax,ay,az$). 
...
then the probability of ``continue'' decreases, and the probability of ``change'' increases. 
...
In summary, in this simplified model, we assume the probability of ``continue'' and ``change''
...
\begin{figure}
    ...
    \caption{The occurrence count of combination (previous action, corresponding axis), sorted from the most popular to least popular.
             Mesh: Thai}
    \label{f:user:newsample}
\end{figure}

Then, we introduce step 2. In this step we need to determine $Pr(A_n |_{current state, A_n != A_p})$
...
At a specific viewpoint $vp = (x, y, z, ax, ay, az)$, if we change the value of $x$, then we stay in the 5-dimension space
\[
...
\begin{align*}
    ...
    =&Pr(x)Pr(y)Pr(z)Pr(ax)Pr(ay)Pr(az)\\
...
corresponding space. For example, the probability of changing $x$
\begin{align*}
...
For example, if the axis to change is $x$ and the 3 neighbors in one direction is
$vp1$, $vp2$, and $vp3$ and the 3 neighbors in
the other direction is $vp4$, $vp5$, $vp6$. 
The probability of moving to the first direction is
\begin{align*}
   =&\frac{Pr(vp1)+Pr(vp2)+Pr(vp3)}
          {Pr(vp1)+Pr(vp2)+Pr(vp3)+Pr(vp4)+Pr(vp5)+Pr(vp6)}\\
   =&\frac{Pr(x1)+Pr(x2)+Pr(x3)}
          {Pr(x1)+Pr(x2)+Pr(x3)+Pr(x4)+Pr(x5)+Pr(x6)}.
\end{align*}.
The reason is that all these viewpoints have same $y$, $z$, $ax$, $ay$, and $az$.
Several neighbors are considered instead of one is because users tend to choose 
the same action continuously for several times. 

There is one exception where choosing direction is not needed.
For the axis we just move along, e.g. for $x$ when the previous 
action is ``Move Left'', we choose the opposite direction with the previous action
since otherwise it becomes ``continue''.
...
\begin{enumerate}
    ...
    \item For other viewpoints, we choose ``continue'' or ``change'' based on the continue probability,
        which is based on {previous action, corresponding coordinate}.
        If we choose ``continue'', the previous action is taken again.
    \item If we choose ``change'', we determine the axis to change based on $Pr(x)$, $Pr(y)$, $Pr(z)$, 
        $Pr(ax)$, $Pr(ay)$, and $Pr(az)$. If the axis is same to the previous one, we choose the opposite 
        action with previous action. Otherwise, we choose the direction based on the popularity of three
