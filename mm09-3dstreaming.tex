...
The ideal way is to record interaction of a large number of users with a variety of meshes under multiple usage scenarios. 
%of testing the mesh streaming system is inviting a large number of real users
...
relatively simple and has a smooth surface, its defect is
...
participants were presented with a user interface mimicking an online
...
The order of the products is randomized to avoid order effects.
...
the viewpoint is equivalent to changing the position of the mesh. 
...
of the bounding box of the mesh. The angles of $\theta_x$, $\theta_y$, and $\theta_z$ change in units of 10$\degree$,
...
Moreover, ($x, y, z, \theta_x, \theta_y, \theta_z$) can be seen as a point in a 6-dimensional space, and
the six axes in this space are $X$, $Y$, $Z$, $AX$, $AY$, $AZ$. 
In this sense, a session is a random walk in the 6-dimensional space.
\begin{figure}
    \centering
...
\section{Predictability and Locality}
...
Some preliminary findings we obtained, however, are interesting themselves.
%and may lead to a potential important research topic. 
We found that 
in the traces we collected, the user actions are somewhat predictable.
...
it means caching can be useful in our scenario.
...
We can see that the most frequently used actions are two revolving (rotating around y-axis) actions. 
...
The $P_{repeat}$ under different combinations of
...
enough samples are required to give reliable results. 
...
\begin{figure}
    ...
    Thai Statue
    \caption{The number of samples on all combinations of the viewpoint and previous action, and the combinations are sorted from the most popular to the least popular.}
...
The distribution is not as skewed as the access count for chunks, 
...
we increase the access count of the visible faces by one. 
Therefore, the access count of a face is the count of viewpoints at 
...
that in the previous two scenarios, but for \textit{Happy Buddha} and \textit{Thai Statue}, a hit rate of 40\%  can be achieved
...
to determine two things: when and what the next action is. In this section we first introduce
...
distribution of the think time. Furthermore, people tend to repeat the same
action quickly, and usually take longer time if they change to another action.
Therefore, whether the next action is repeating the last action is also a factor to be considered.
...
of viewpoints during determining think time.
...
According to the observation that the previous actions highly affect the next action
...
Second, due to the large sample space and limited number of samples, most of the states have only one sample
(See Figure \ref{f:user:sample_size_2}). 
In other words, in the transition matrix, the next action for most states is determined. 
...
Therefore, the previous action is chosen as one of the most important factors. 
Based on the relation to the previous action, we categorized actions to three groups: 
...
only depend on (previous action, corresponding coordinate).
...
then we stay in the 5-dimensional space
\[
...
Hence, we set the probability of changing a value of an axis to be proportional to the probability of staying in the
...
\end{align*}
Similarly, we can know the probability of changing other coordinates. Intuitively, we can think of $Pr(x)$ as 
the popularity of the current $x$ value. The above equation means that we tend to change the coordinate having low
popularity.
...
Then, we assume the probability to go in one direction is proportional to the popularity of that direction.
...
\end{align*}
Notice all these viewpoints have the same $y$, $z$, $\theta_x$, $\theta_y$, and $\theta_z$ values, so they are not in
...
First, we compare the unconditional probability (the probability without consideration of any condition, such as the
current viewpoint or the previous action) of each action in the traces. 
From Figure \ref{f:user:frequency_comp}, we can see that the probability of each action in the traces we generated
fits well with that in the real traces. 
This result is impressive because we never consider unconditional probability in our model, but  
...
the synthetic traces we generated have similar $P_{repeat}$ under all conditions.
\begin{figure}[htdp!]
    \centering
...
axes. Among the six axes, only the $Y$ axis has a slightly higher error. The synthetic traces tend to stay in the
center ($0$) more often. The correlation between these two sets of values, however, is still close to $1$. 
...
\begin{figure}[htdp!]
    ...
    \end{tabular}
    \caption{Comparison of distribution of viewpoints on different axes.}
...
If the Markov chain method is used, the synthetic traces should have the same transition probability as the real trace.
...
We only choose some states with a sufficient number of samples to ensure the transition probabilities
...
Moreover, according to our observation, the traces we generated have much less
...
These synthetic traces are similar to the real traces in the properties we compared.
...
designing progressive mesh streaming systems and rendering systems.
...
our findings above, despite being limited to a single usage scenario, 
points to a potentially important new research topic.  
Our study in this thesis establishes the first step towards that direction.
