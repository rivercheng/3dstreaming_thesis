...
We log the user's actions while they interact and view the meshes in a mock online shop.  We present the analysis of these traces in this paper, and highlight findings that are significant to the design of efficient and scalable progressive mesh streaming systems.  In particular, we found that (i) in certain scenarios, user actions are highly predictable, making pre-fetching useful in these cases; (ii) users' viewpoint concentrates on part of the meshes, making caching these popular spots useful. %We also develop a simple user model, which can be used to generate synthetic traces for evaluating large-scale progressive mesh streaming systems.

%\section{Background}
%
%No similar study has been done on user bahaviour of users with 
%progressive streaming of 3D model wich we know of.  \todo{not very useful as its own section.  Perhaps remove?}

\begin{figure}[htp]
\centering
\includegraphics[height=0.18\textwidth]{figs/thai}
\includegraphics[height=0.18\textwidth]{figs/dragon}
\includegraphics[height=0.18\textwidth]{figs/happy} 
...
\section{User Study}
%\textbf{Meshes}. We use three 3D meshes that are freely available at
%the Stanford 3D Scanning Repository\footnote{http://graphics.stanford.edu/data/3Dscanrep/} for our experiment, namely,   
%\textit{Thai Statue}, \textit{Dragon}, and \textit{Happy Buddha}.  The meshes are shown in Figure
%\ref{fig:3dmodels}. Each of these meshes have unique characteristics. 
%\textit{Happy Buddha} has distinct
%front and back faces. \textit{Dragon} is horizontally oriented and has no such distinction.  \textit{Thai Statue} is vertically oriented and has three components: the dragons head at the
%bottom, the elephants head in the middle, and a human figure at the top.
%This statue has three symmetric sides, so it does not have a distinct front or back, either.
...
We replicate each mesh above twice, generating nine
...
\begin{figure}[htp!]
\begin{center}
 \epsfig{file=buddha.eps, height=0.2\textwidth}
 \epsfig{file=faceCache.eps, width=0.2\textwidth}
