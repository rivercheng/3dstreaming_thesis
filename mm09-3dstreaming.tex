...
Figure \ref{fig:UnActionEventFrequency}
(a) shows the frequency of occurrences of 12\footnote{We ignore RESET and QUIT 
in this figure because they have the lowest probability.}
different operations on the traces from each of 9 meshes,
...
\begin{figure}[htdp!]
    \centering
    \epsfig{file=cont_prob_9.eps, width=0.7\textwidth}
...
From Figure \ref{}, we can see that the second latest action does not affect the 
distribution of next action much. The prediction based on previous action and the
one based on previous two actions are same in almost all cases. Therefore, we conclude
that considering only the previous action is enough for the prediction.
...
\begin{figure}
    \centering
    \epsfig{file=reduction.eps, width=0.8\textwidth}
