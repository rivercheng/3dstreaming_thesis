\chapter{Study of User Behaviors}
\label{c:user}
...
A viewpoint indicates the from where and to which direction users view the mesh.
...
Then, we rotate the mesh $ax$ degrees around $x$ axis, $ay$ degrees around $y$ axis,
and $az$ degrees around $z$ axis. 
...
\begin{figure}
    \centering
    \epsfig{file=transition.eps, width=0.5\textwidth}
...
we categorized the actions into two types: \textit{continue}, \textit{non-continue}. 
...
\begin{figure}[htdp!]
    ...
    \caption{The probability of continuing the previous action for 9 meshes.}
    \label{f:user:cont_prob}
\end{figure}

\begin{figure}
    \centering
    \epsfig{file=thai_2.eps, width=0.5\textwidth}
...
so we finally define the state to be ``($x, y, z, ax, ay, az, A_p$).'' 
(See Figure \ref{f:user:reduction}(B)).
\begin{figure}
    \centering
    \epsfig{file=reduction.eps, width=0.6\textwidth}
    \caption{(A), State = (previous viewpoint, current viewpoint); (B), State = ($x, y, z, ax, ay, az, A_p$)}
...
state ``($x,y,z,ax,ay,az,A_p$)''. Due to the limited number of samples, we cannot obtain the reliable
...
In summary, in this simplified model, we assume that the probability of ``continue'', ``reverse'', and ``change''
...
At a specific viewpoint $Vp = (x, y, z, ax, ay, az)$, if we change the value of $x$, 
...
Hence, we set the probability of changing a value of axis to be proportional to the probability of staying in the
...
\begin{enumerate}
    ...
    \item We randomly select a think time from two different distributions 
        based whether the action is ``continue'' or not (see Section \ref{ss:user:thinktime}).
...
We generated 10000 random traces following the simplified model we introduced in the last section.
...
From Figure \ref{f:user:frequency_comp}, we can see the traces we generated fit well with the real traces. 
...
Second, we compare the probability of continuing the previous action, including unconditional continue probability
and the continue probability under different previous action. We can see from Figure \ref{f:user:continue_prob_comp},
the synthetic traces we generated has similar continuing probability under all conditions.
\begin{figure}[htdp!]
    \centering
    \epsfig{file=continue_prob_comp.eps, width=0.5\textwidth}
    \caption{Comparison of unconditional continue probability and continue probability under different previous actions.}
    \label{f:user:continue_prob_comp}
\end{figure}

Third, we compare how the viewpoints distributed on each axis, 
i.e. the value of $Pr[x]$, $Pr[y]$, $Pr[z]$, $Pr[ax]$, $Pr[ay]$, and $Pr[az]$. 
Figure \ref{f:user:axis_distribution} shows that the synthetic traces we generated has similar distributions on all 
axises. Among the six axises, only the $Y$ axis has a slightly higher error. The synthetic traces tends to stay in the
center ($0$) more often. The correlation between these two set of values, however, is still close to $1$. 
Again, this result is interesting. Although we have considered these probabilities in the selection new action when
``change'' is selected, the majority of selection (recall that generally more than 80\% of actions are ``continue'')
...
The final comparison is on the transition probability on certain state, $Pr(A_n|_{(x, y, z, ax, ay, az, A_p)}$. 
...
of real traces, the value of comparison based on states is limited. According to the results we have, however, the simplified model
works well.

\section{Conclusion}
We conclude this chapter by showing the most significant findings we have.
First, the analysis of user traces reveals that locality exists in both data and viewpoint access. By
...
Third, we proposed a model to generate synthetic traces based on real traces we collected. These synthetic traces have similar
properties with the real traces and can be used in simulations to measure system performance.
