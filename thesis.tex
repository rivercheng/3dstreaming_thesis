...
\label{c:intro}
    ...
    %\subsubsection*{Some concepts about mesh}
    ...
    %Here are some important concepts about mesh without
    %giving rigorous definition. 
...
\begin{comment}
    Two vertices are \emph{adjacent} if there is an edge between them,
    and two faces are \emph{adjacent} if they share an edge. The
    \emph{orientation} of a polygon is the cyclic order (clockwise or
    counter-clockwise) of the incident vertices. If two faces are
    adjacent and the two vertices in the common edge are
    in the opposite order, their orientations are \emph{compatible}.
    If there is an arrangement of all the face orientations in a mesh
    such that all pairs of adjacent faces are compatible, this mesh is
    \emph{orientable}. A compatible mesh with such arrangement is
    called an \emph{oriented mesh} (see figure \ref{orient}). As we
    can see, an orientable manifold mesh is simpler than non-manifold
    non-orientable meshes. It is worth noting that many algorithms
    introduced later can only deal with orientable manifold meshes. A
    typical orientable manifold triangle mesh has following
    properties: its vertex number is almost twice as many as the
    triangle number; its edge number is almost 1.5 as many as the
    triangle number; the average valence value of a vertex is about 6.
    More detail about the triangle meshes can be seen in Chapter 2 of
    \cite{381198}.
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{orient.eps}
\caption[Orientable and non-orientable mesh]{(a) is a orientable
and oriented mesh. (b) is a orientable but not oriented mesh. (c)
is a non-orientable mesh. \label{orient}}
\end{figure}
\end{comment}
    
    %\subsubsection*{Associate properties with mesh}
...
\begin{comment}
    \subsubsection*{Mesh simplification and progressive mesh}
    Mesh simplification means to transform a given polygonal mesh into
    another mesh with fewer faces, edges and vertices, and the simpler
    mesh should preserve specific properties of the original mesh as
    much as possible. A lower resolution mesh is useful in faster
    rendering or transmission. The algorithms to simplify mesh are
    named \emph{mesh decimation}. They can be categorized into three
    classes: Re-sampling algorithms, Vertex clustering algorithms, and
    Incremental decimation algorithms.

    Re-sampling algorithms re-sample a given mesh into a new
    mesh with some attractive property, such as semi-regular
    connectivity, for some specific compress algorithms. 
    Re-sampling algorithms try to keep the shape of the original mesh but not the topology, 
    so they are usually used in lossy compression algorithms, 
    which are out of the scope of this paper.

    The idea of Vertex clustering is as follows: given a certain
    approximation tolerance $\varepsilon$, we partition the bounding
    space of the mesh into cells with diameter smaller than
    $\varepsilon$. Then for each cell we generate a representative
    vertex position for all the vertices in this cell. In another
    word, all the vertices, edges and faces entirely in one cell are
    degenerated to one vertex (see figure \ref{simplification} (a)).
    Vertex clustering is simple and robust but often provide worse
    model quality than incremental decimation.

    Incremental decimation algorithms remove vertex one by one. In
    each step, a vertex whose removal cause the minimal
    reduction to the quality of the mesh is chosen. Usually there are two ways to
    remove a vertex: \emph{vertex removal}\label{vertex_removal} and
    \emph{edge collapse}\label{edge_collapse}. Vertex removal is to
    delete a vertex and re-triangulate the hole caused by the removal
    of this vertex (see figure \ref{simplification} (b)). Edge collapse operation
    chooses two adjacent vertices $p$ and $q$,  and then moves them both
    to the same point on the edge between them (see figure
    \ref{simplification}(c)). A special kind of edge collapse is
    called \emph{half-edge collapse}, which always move one vertex $p$
    to one of its neighbors $q$. Actually, half-edge collapse is
    a special case of vertex removal too, and the  difference is half-edge collapse has 
    no freedom to choose a triangulation as in normal vertex
    removal. Incremental decimation can provide optimal mesh based on
    some specific criteria, but the computational complexity is always
    much higher than vertex clustering.
\begin{figure}
\centering
\includegraphics[width = 0.8\textwidth]{simplification.eps}
\caption[Mesh simplification]{Mesh simplification: (a) vertex
clustering, (b) vertex removal and vertex insertion, and (c) edge collapse
and vertex split\label{simplification}}
\end{figure}
    In incremental decimation, the operation done can always be recorded 
    so that its inverse can be taken to restore the original mesh. The
    inverse of vertex removal is \emph{vertex insertion} and the
    inverse of edge collapse is \emph{vertex split} (see figure
    \ref{simplification}(b,c)). Accordingly, we have \emph{restricted
    vertex split} as the inverse of half edge collapse. More details 
    about mesh simplification can be seen in the tutorial of Gotsman,
    Gumhold and Kobbelt \cite{gotsman-simplification}.

    Suppose we obtain a base model after a series of incremental
    decimation and record all the inverses, and then we transfer the
    base model followed by the inverses with the reverse order. At the
    receiver side, the model can be rendered anytime after the base
    model has been received and the quality of model depends on how
    much inverses received. Therefore, this kind of mesh, a base mesh
    plus a series of refinement operations, is called
    \emph{progressive mesh}, which will be introduced in more detail 
    in section \ref{progressive_mesh}.
\end{comment}

    \subsection{Point-based Model}\label{point_based}
\begin{comment}
    Triangle meshes always try to keep the accurate topology information, 
    so they may be not suitable for some cases. 
    For a model with complicate topology, 
    it is very expensive to keep the topology information, 
    while the topology information is not necessary in some cases. 
    For example, the resolution of models becomes higher and higher 
    due to the improvements of laser scanner, 
    but the resolution of the screen grows much slower. 
    Therefore, many triangles in a high resolution model may be 
    rendered into one pixel in the screen. 
    In this case, all the expensive connectivity information 
    are not necessary in the rendering, 
    and one point is enough to represent this part. 
    Moreover, the topology information of dynamic 3D models always changes, 
    hence a consistent topology is not necessary to be kept. 
    Therefore, point based models have more flexibility for dynamic 3D objects.
\end{comment}
...
\begin{figure}
\centering
\includegraphics[width=0.4\textwidth]{qsplat.eps}
\caption{Bounding Sphere Hierarchy of QSplat: the curves and horizontal arrows represent the storage order.}
\label{qsplat}
\end{figure}
    \end{comment}
    ...
    %They are both introduced in this section.
    ...
    %connectivity coding has been researched in depth and near optimal result has been reached,
    %although actually the latter two dominate in a mesh. Now, more
    %studies begin to concentrate in geometry coding than before. 
    ...
    \begin{comment}
    In VRML ASCII representation, a polygonal mesh is represented by
    an \emph{Indexed-FaceSet} node with a \emph{coord} array, and a
    \emph{coordIndex} array. For example, a representation of a
    tetrahedron is as follows:
    \begin{quote}
    \begin{verbatim}
#VRML V2.0 utf8 Shape {
  geometry IndexedFaceSet {
    ...
    \end{verbatim}
    \end{quote}
    As we can see, each face is represented with the indices of the
    vertices, followed by -1 as a face separator. 
    For a mesh with $v$ vertices, one index needs ${\lceil}log_{2}v\rceil$ bits. For a
    triangular mesh, every face needs $3{\lceil}log_{2}v{\rceil}$ bits. 
    Therefore, the total cost is $3{\lceil}flog_{2}v{\rceil}$,
    where $f$ represents the number of faces. Since in a typical
    triangular mesh $v \approx 2f$, the total cost can be represented
    as $6v{\lceil}log_{2}v{\rceil}$ or $6{\lceil}log_{2}v{\rceil}$
    bpv (bit per vertex).
\end{comment}
    ...
    %Since the number of faces is
    %about twice as many as the number of vertices, 
    %some vertices have to be repeatedly indexed. 
    ...
    %In his work, a first-in-first out vertex buffer is used to store up to 16
    %recently visited vertices. Then vertices saved in the buffer can
    %be represented with 4 bit buffer indices instead of much longer
    %global indices. 
    Further, Chow \cite{267103} introduced a method to
    decompose a mesh into triangle strips. 
    %In their paper, a mesh is decomposed into triangle strips from boundary to the center. 
    Bajaj, Pascucci and Zhuang \cite{789628} presented a coding method with layered
    decomposition. 
    %A triangular mesh is decomposed into several
    %concentric layers of vertices and then the triangles between each
    %pair of adjacent vertex layers is constructed to triangle layers.
    Typically, a triangle layer is a generalized triangle strip. 
    %Then
    %the connectivity coding is reduced to coding of the number of
    %vertex layers, the number of vertices in each layer, and the
    %generalized triangle strip in each triangle layer. This method is
    %reported to be able to encode the connectivity in about 1.40-6.08
    %bpv.
    \begin{figure}[ht]
    \centering
    ...
    %First, a vertex spanning tree is
    %selected and then the mesh is cut along the edges of this tree. A
    %planar polygon is generated and the dual of the triangles is a tree too (see Figure \ref{surgery}). By
    %encoding these two trees with run-length method, the connectivity
    %of a manifold mesh can be coded in 2.48 - 7.0 bpv. 
...
\begin{comment}
    \begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{surgery.eps}
    \caption{(A)An octahedron mesh, (B)its vertex spanning tree, and (C) the cut and flattened mesh with its triangle spanning tree shown by dashed lines.}\label{surgery}
    \end{figure}
\end{comment}
    ...
    %With
    %the valences, the connectivity information can be re-constructed in
    %the receiver side. As we know, the average valence of a vertex in
    %a typical manifold mesh is about 6 and the distribution is quite compact,
    %so the entropy of valence is small. Therefore, the valences
    %can be coded effectively with arithmetic coder and the cost is
    %less than 1.5 bpv on average. 
    ...
    coder. 
    %Prediction schemes can be categorized into several classes:
    %\emph{delta prediction}, \emph{linear prediction},
    %\emph{parallelogram prediction}, and \emph{second-order
    %prediction}. %We do not introduce them due to the limited space.
    ...
    %\ref{edge_collapse} in page \pageref{edge_collapse}). 
