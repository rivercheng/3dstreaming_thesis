...
\end{comment}
    ...
    %it is important to examine the objects to be streamed first. 
    %High resolution 3D models are often obtained from 3D scanning, 
    %modeling softwares, and other
    %sources. 
    %3D models are represented with certain models and often
    ...
    %Therefore, the polygonal mesh is firstly introduced.

    %\subsubsection{Polygonal Mesh}
...
\end{comment}
    ...
    %Besides point-based meshes, which is better in some cases.
...
\end{comment}
    ...
    %, whose difference from valence-driven is that in a
    %specific order, what kind of operations
    %to build new triangles instead of valences is recorded. 
    %Gumhold and
