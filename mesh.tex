...
If $i < T_{d}$, %then for any $k \geq 0$,
\begin{eqnarray*}
S_i = i.
\end{eqnarray*}
Otherwise, 
...
%$Pr(R_i \leq t) = 1 - p^{n_{i,t}+1}$.
    Pr(R_i < t) &=& \left\{
   \begin{array}{ll}
        1 - p^{n_{i,t}}                  & \mbox{if } (t-S_i) \mod T_d = 0  \\
        1 - p^{n_{i,t}+1}                & \mbox{otherwise}
   \end{array}
   \right.
\end{eqnarray*}
\end{lemma}
    ...
    $p^{n_{i,t}+1}$. Then, we have $Pr(R_i \leq t) = 1 - p^{n_{i,t}+1}$.
    Considering $Pr(R_i < t) = Pr(R_i \leq t) - Pr(R_i = t)$ and Lemma~\ref
    {l:model:preqt}, 
...
\end{figure}
    ...
    %packet $0$ to packet $b-1$ has $n+1$ chances to
    ...
    %Intuitively, it is because that packet $0$ will be sent after 
    %packet $1$ to 
    packet $T_d - 1$ (See Figure \ref{f:model:comp}).
    %if is lost in the first transmission (See Figure \ref{f:model:comp}). 
...
%using the meshes \textit{Horse} and \textit{Thai Statue}, averaged over 10 runs, using the \textsf{UDP-High} trace.}
%\label{f:horse_thai_1}
\label{f:model:other_time_comp_2}
\end{figure*}
  %we can interpret the meaning of this metric
    \paragraph*{Single Run}
    %Figure~\ref{f:1_run} 
    Figures~\ref{f:model:other_time_comp_2}(a)-(b) shows the predicted values of receiving time and decoding time compared to the measured ones 
...
\centering
\epsfig{file = figures/plots/\tracea/\mesh_h/10/quality_curve_a_s.eps, width = \picwidth, angle=270}
\caption[Predicted quality curve and measured quality curve from our experiments.]{Predicted quality curve and measured quality curve using Hausdorff distance as quality metric, averaged over 10 runs.}
%\label{f:hausdorff}
\label{f:model:hausdorff}
\end{figure*}
...
\end{tabular}
\caption[Predicted quality curve and measured quality curve from our experiments.]{Predicted quality curve and measured quality curve from two different single runs.}
%\label{f:hausdorff}
\label{f:model:1run}
\end{figure*}
    \paragraph*{Single Run}
    ...
    (see Figure \ref{f:model:1run}).  The measured quality is lower for the second sample
